...
from PyQt5.QtGui import QImage, QPixmap, QBrush, QPen, QColor, QPainter, QCursor, QPainterPath, QFont, QFontDatabase
from PyQt5.QtWidgets import QFrame, QLabel, QGridLayout, QGraphicsOpacityEffect, QSizePolicy, QToolButton
from PyQt5.QtWidgets import QActionGroup, QVBoxLayout, QWidget, QApplication, QHBoxLayout, QProgressBar
from client.seged.qt_lib.q_subprocessor import QSubprocessor
from client.seged.qt_lib.qx_iconbutton import QXIconButton
from client.seged.qt_lib.qx_mainwindow import QXMainWindow
from client.seged.qt_lib.q_dark_palette import QDarkPalette
from client.seged.qt_lib.q_action_ex import QActionEx
from client.seged.qt_lib.helpers import QImage_from_np, QSize_to_np, QPoint_to_np, QImage_to_np, QPoint_from_np
from client.seged.qt_lib.cursor_db import QCursorDB
from client.seged.qt_lib.icon_db import QIconDB
from client.seged.qt_lib.image_db import QImageDB
from client.seged.qt_lib.string_db import QStringDB
from client.seged import pathex
from core.image_processing.processing_helpers import dist_to_edges, normalize_channels
from core.image_processing.seg_polygons import SegIEPolys, SegIEPolyType
from core.image_processing.png_proc import DflPng

packed_faceset_filename = 'faceset.pak'


def dflimg_load(filepath):
    return DflPng.load(filepath)
...
class QUIConfig:
    icon_q_size  = None
    icon_spacer_q_size = None
    preview_bar_icon_q_size = None
...
class ImagePreviewSequenceBar(QFrame):
    def __init__(self, preview_images_count, icon_size):
        ...
        black_q_img = QImage(np.zeros((icon_size, icon_size, 3)).data, icon_size, icon_size, 3 * icon_size,
    ...
    def update_images(self, prev_imgs=None, next_imgs=None):
        ...
        for i, img in enumerate(prev_imgs):
            self.prev_img_conts[i].setPixmap(
                QPixmap.fromImage(QImage_from_np(img)) if img is not None else self.black_q_pixmap)
...
class CanvasConfig:
    ...
    def __init__(self,
        ...
        if color_schemes is None:
            color_schemes = [
                ColorScheme(QColor(192, 0, 0, alpha=0), QColor(192, 0, 0, alpha=72), QColor(192, 0, 0), 2,
                            QColor(255, 255, 255), QCursorDB.cross_red),
                ColorScheme(QColor(0, 192, 0, alpha=0), QColor(0, 192, 0, alpha=72), QColor(0, 192, 0), 2,
                            QColor(255, 255, 255), QCursorDB.cross_green),
                ColorScheme(QColor(0, 0, 192, alpha=0), QColor(0, 0, 192, alpha=72), QColor(0, 0, 192), 2,
                            QColor(255, 255, 255), QCursorDB.cross_blue),
            ]
...
class QCanvasControlsLeftBar(QFrame):
    def __init__(self):
        ...
        self.btn_poly_type_include_act = QActionEx(QIconDB.poly_type_include, QStringDB.btn_poly_type_include_tip,
                                                   shortcut='Q', shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_poly_type_exclude_act = QActionEx(QIconDB.poly_type_exclude, QStringDB.btn_poly_type_exclude_tip,
                                                   shortcut='W', shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_undo_pt_act = QActionEx(QIconDB.undo_pt, QStringDB.btn_undo_pt_tip, shortcut='Ctrl+Z',
                                         shortcut_in_tooltip=True, is_auto_repeat=True)
        btn_undo_pt.setDefaultAction(self.btn_undo_pt_act)
        btn_undo_pt.setIconSize(QUIConfig.icon_q_size)

        btn_redo_pt = QToolButton()
        self.btn_redo_pt_act = QActionEx(QIconDB.redo_pt, QStringDB.btn_redo_pt_tip, shortcut='Ctrl+Shift+Z',
                                         shortcut_in_tooltip=True, is_auto_repeat=True)
        ...
        self.btn_delete_poly_act = QActionEx(QIconDB.delete_poly, QStringDB.btn_delete_poly_tip, shortcut='Delete',
        ...
        self.btn_pt_edit_mode_act = QActionEx(QIconDB.pt_edit_mode, QStringDB.btn_pt_edit_mode_tip,
                                              shortcut_in_tooltip=True, is_checkable=True)
...
class QCanvasControlsRightBar(QFrame):
    def __init__(self):
        ...
        self.btn_poly_color_red_act = QActionEx(QIconDB.poly_color_red, QStringDB.btn_poly_color_red_tip, shortcut='1',
                                                shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_poly_color_green_act = QActionEx(QIconDB.poly_color_green, QStringDB.btn_poly_color_green_tip,
                                                  shortcut='2', shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_poly_color_blue_act = QActionEx(QIconDB.poly_color_blue, QStringDB.btn_poly_color_blue_tip,
                                                 shortcut='3', shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_view_baked_mask_act = QActionEx(QIconDB.view_baked, QStringDB.btn_view_baked_mask_tip, shortcut='4',
                                                 shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_view_xseg_mask_act = QActionEx(QIconDB.view_xseg, QStringDB.btn_view_xseg_mask_tip, shortcut='5',
                                                shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_view_xseg_overlay_mask_act = QActionEx(QIconDB.view_xseg_overlay,
                                                        QStringDB.btn_view_xseg_overlay_mask_tip, shortcut='`',
                                                        shortcut_in_tooltip=True, is_checkable=True)
        ...
        self.btn_view_lock_center_act = QActionEx(QIconDB.view_lock_center, QStringDB.btn_view_lock_center_tip,
                                                  shortcut_in_tooltip=True, is_checkable=True)
...
class QCanvasOperator(QWidget):
    def __init__(self, cbar):
        ...
        self.cbar.btn_pt_edit_mode_act.toggled.connect(
            lambda is_checked: self.set_pt_edit_mode(PTEditMode.ADD_DEL if is_checked else PTEditMode.MOVE))
        self.cbar.btn_view_lock_center_act.toggled.connect(
            lambda is_checked: self.set_view_lock(ViewLock.CENTER if is_checked else ViewLock.NONE))
    ...
    def initialize(self, img, img_look_pt=None, view_scale=None, ie_polys=None, xseg_mask=None, canvas_config=None):
        ...
        if xseg_mask is not None:
            ...
            xseg_mask = normalize_channels(xseg_mask, 1)
    ...
    def finalize(self):
        if self.initialized:
            ...
            self.last_state = Sn(
                op_mode=self.op_mode if self.op_mode in [OpMode.VIEW_BAKED, OpMode.VIEW_XSEG_MASK] else None,
    ...
    def get_poly_pt_id_under_pt(self, poly, cli_pt):
        w = np.argwhere(
            npla.norm(cli_pt - self.img_to_cli_pt(poly.get_pts()), axis=1) <= self.canvas_config.pt_select_radius)
    ...
    def get_poly_edge_id_pt_under_pt(self, poly, cli_pt):
        ...
        if len(cli_pts) >= 3:
            edge_dists, projs = dist_to_edges(cli_pts, cli_pt, is_closed=True)
    ...
    def get_poly_by_pt_near_wire(self, cli_pt):
        ...
        for poly in reversed(self.ie_polys.get_polys()):
            ...
            if len(pts) >= 3:
                ...
                edge_dists, _ = dist_to_edges(cli_pts, cli_pt, is_closed=True)
    ...
    def img_to_cli_pt(self, p):
        return (
                       p - self.get_img_look_pt()) * self.get_view_scale() + self.get_cli_center_pt()  # QSize_to_np(self.size())/2.0
        ...
        if self.op_mode == OpMode.VIEW_BAKED:
            ...
            for i, poly in enumerate(polys):
                ...
                if pt_remove_cli_pt is not None:
                    ...
                    qp.drawLine(*(pt_remove_cli_pt + np.float32([-pt_rad_x2, -pt_rad_x2])),
                                *(pt_remove_cli_pt + np.float32([pt_rad_x2, pt_rad_x2])))
                    qp.drawLine(*(pt_remove_cli_pt + np.float32([-pt_rad_x2, pt_rad_x2])),
                                *(pt_remove_cli_pt + np.float32([pt_rad_x2, -pt_rad_x2])))
...
class LoaderQSubprocessor(QSubprocessor):
    ...
    class Cli(QSubprocessor.Cli):
        def process_data(self, data):
            idx, filename = data
            dflimg = dflimg_load(filename)
            if dflimg is not None and dflimg.has_metadata:
                ie_polys = dflimg.xpolys
...
class MainWindow(QXMainWindow):
    def __init__(self, input_dirpath, cfg_root_path):
        ...
        self.loader_subprocessor = LoaderQSubprocessor(
            image_paths=pathex.get_image_paths(input_dirpath, return_path_class=True),
    ...
    def on_loader_finish(self, image_paths, image_paths_has_ie_polys):
        ...
        self.set_has_ie_polys_count(
            len([1 for x in self.image_paths_has_ie_polys if self.image_paths_has_ie_polys[x] == True]))
    ...
    def update_cached_images(self, count=5):
        ...
        for image_path in self.image_paths[:count] + self.image_paths_done[-count:]:
            ...
            if image_path not in d:
                img = dflimg_load(image_path)
                if img is not None:
                    d[image_path] = img.face_centered_image

    def load_image(self, image: DflPng, image_path):
        img = self.cached_images.get(image_path, None)
        if img is None:
            self.cached_images[image_path] = image.face_centered_image
        return img
    ...
    def canvas_initialize(self, image_path, only_has_polys=False):
        ...
        dflimg = dflimg_load(image_path)
        if not dflimg or not dflimg.has_metadata:
            return False

        ie_polys = dflimg.xpolys
        xseg_mask = dflimg.xmask
        img = self.load_image(dflimg, image_path)
        if img is None:
            return False
    ...
    def initialize_ui(self):
        ...
        image_bar = self.image_bar = ImagePreviewSequenceBar(preview_images_count=9,
                                                             icon_size=QUIConfig.preview_bar_icon_q_size.width())
        image_bar.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)

        btn_prev_image = QXIconButton(QIconDB.left, QStringDB.btn_prev_image_tip, shortcut='A',
                                      click_func=self.process_prev_image)
        btn_prev_image.setIconSize(QUIConfig.preview_bar_icon_q_size)

        btn_next_image = QXIconButton(QIconDB.right, QStringDB.btn_next_image_tip, shortcut='D',
                                      click_func=self.process_next_image)
        btn_next_image.setIconSize(QUIConfig.preview_bar_icon_q_size)

        btn_delete_image = QXIconButton(QIconDB.trashcan, QStringDB.btn_delete_image_tip, shortcut='X',
                                        click_func=self.trash_current_image)
...
def start_segmentation_editor(input_dirpath):
    """
    returns exit_code
    """
    print("Running XSeg editor.")

    if path_contains(input_dirpath):
        # io.log_info(f'\n{input_dirpath} contains packed faceset! Unpack it first.\n')
        print("Input dir contains packed faceset! Unpack it first!")
        return 1

    root_path = Path(__file__).parents[2] / "client/seged/qt_lib/graphics"
    ...
    QFontDatabase.addApplicationFont(str(root_path / 'fonts' / 'NotoSans-Medium.ttf'))
    ...
    QIconDB.initialize(root_path / 'icons')
    QCursorDB.initialize(root_path / 'cursors')
    QImageDB.initialize(root_path / 'images')
